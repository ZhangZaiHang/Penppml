...
*! Extension of jwdid command from Fernando Rios-Avila by Arne Nagengast for the project "Staggered Difference-in-Differences in Gravity Settings: Revisiting the Effects of Trade Agreements" by Arne Nagengast and Yoto Yotov
program jwdid, eclass
	version 14
	syntax varlist [if] [in] [pw], Ivar(varname) Tvar(varname) Gvar(varname) [never group method(name) Fe(string) exovar(string) addint(varlist) trendvar(varlist) trendt trendg trendij restriction(name) binT(numlist min=1 max=1 >0 integer) interval(numlist min=1 max=1 >1 integer) binpre(numlist min=1 max=1 <0 integer) ifcond(varname)] [cvar(string)] [tol(string)] [boot]
	marksample  touse
	...
	***	
	qui: capture drop xvarfe
	qui: gen xvarfe = 0
	local xvarfe_counter=1
	foreach i of local glist {
		...
		if "`interval'"=="" & "`addint'"=="" & "`binT'"=="" & "`binpre'"=="" {			
			...
			if "`never'"=="" & "`restriction'"=="" & "`boot'"!="" {
				...
				foreach j of local tlist {
					if `j'>=`i' {
						local xvar `xvar' i`i'.`gvar'#i`j'.`tvar'#c.__tr__
						qui: replace xvarfe=`xvarfe_counter' if __tr__ & `gvar'==`i' & `tvar'==`j'
						local xvarfe_counter = `xvarfe_counter'+1
					}
				}
			}
		}
	...
	** for xs	
	...
	if "`method'"=="" {
		if "`group'"=="" & "`fe'"==""  {
			reghdfe `y' `exovar' `xvar'   `otxvar'	 	///
				if `touse' [`weight'`exp'], abs(`ivar' `tvar' `trend') cluster(`cvar') keepsingletons
		}
		else if "`group'"=="" & "`fe'"!="" & "`boot'"=="" {
			reghdfe `y' `exovar' `xvar'   `otxvar'	 	///
				if `touse' [`weight'`exp'], abs(`ivar' `tvar' `fe' `trend') cluster(`cvar') keepsingletons
		}
		else if "`group'"=="" & "`fe'"!="" & "`boot'"!="" {
			reghdfe `y' `exovar' `xvar'   `otxvar'	 	///
				if `touse' [`weight'`exp'], abs(`ivar' `tvar' `fe' `trend', savefe) cluster(`cvar') keepsingletons
		}		
		else {		 
			reghdfe `y' `exovar' `xvar'  `x'  `ogxvar' `otxvar'	  ///
		...
		if "`group'"=="" & "`fe'"==""  {
			ppmlhdfe `y' `exovar' `xvar'   `otxvar'	 ///
				if `touse' [`weight'`exp'], abs(`ivar' `tvar' `trend') cluster(`cvar') keepsingletons d tol(`tol')
		}
		else if "`group'"=="" & "`fe'"!="" & "`boot'"=="" {
			ppmlhdfe `y'   `exovar' `xvar'   `otxvar'	 ///
				if `touse' [`weight'`exp'], abs(`ivar' `tvar' `fe' `trend') cluster(`cvar') keepsingletons d tol(`tol')
		}
		else if "`group'"=="" & "`fe'"!="" & "`boot'"!="" {
			ppmlhdfe `y'   `exovar'  	 ///
				if `touse' [`weight'`exp'], abs(`ivar' `tvar' `fe' xvarfe, savefe) cluster(`cvar') keepsingletons d tol(`tol')
		}
		else {		 
			ppmlhdfe `y' `exovar' `xvar'  `x'  `ogxvar' `otxvar'	   ///
			if `touse' [`weight'`exp'], abs(`gvar' `tvar' `trend') cluster(`cvar') keepsingletons d tol(`tol')
		}
	}	
	else {
